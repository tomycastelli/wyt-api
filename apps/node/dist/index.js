...
const coins_service = new CoinsService(process.env.POSTGRES_URL ?? "", process.env.COINGECKO_API_KEY ?? "");
...
coins_routes.get("/:blockchain", arktypeValidator("query", type({ "page?": "number", "name_search?": "string" })), async (c) => {
    const blockchain = c.req.param("blockchain");
    const { page, name_search } = c.req.valid("query");
    const page_size = 30;
    const savedCoins = await coins_service.getCoinsByBlockchain(blockchain, page ?? 1, page_size, name_search);
...
coins_routes.get("candles/:coin_name/:candle_type", arktypeValidator("param", type({ candle_type: "'hourly'|'daily'", coin_name: "string" })), arktypeValidator("query", type({ from: milisecond_timestamp, to: milisecond_timestamp })), async (c) => {
    const { candle_type, coin_name } = c.req.valid("param");
    const { from, to } = c.req.valid("query");
    const { id: coin_id } = await coins_service.getCoinByName(coin_name);
    const candles = await coins_service.getCandlesByDate(candle_type, coin_id, from, to);
