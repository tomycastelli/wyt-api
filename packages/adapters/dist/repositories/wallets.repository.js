...
export class WalletsPostgres {
    ...
    async saveWallet(coined_wallet) {
        await this.db.transaction(async (tx) => {
            ...
            if (coined_wallet.coins.length > 0) {
                ...
                await tx.insert(schema.walletCoins).values(coined_wallet.coins.map((c) => ({
                    coin_id: c.coin.id,
    ...
    async getWallet(address, blockchain) {
        const wallet = await this.db.query.wallets.findFirst({
            where: (wallets, { eq, and }) => and(eq(wallets.address, address), eq(wallets.blockchain, blockchain)),
            with: {
                walletCoins: {
                    with: {
                        coin: {
        ...
        const coined_wallet = {
            ...
            coins: wallet.walletCoins.map((c) => ({
                coin: c.coin,
                coin_address: c.coin.contracts.find((ct) => ct.blockchain === blockchain).contract_address,
                value: c.value,
            })),
        };
    ...
    async getWalletsByBlockchain(blockchain, wallets_page) {
        ...
        const coined_wallets = wallets.map((w) => ({
            ...w,
            native_coin: native_coin,
    ...
    async saveTransactions(transactions) {
        ...
        await this.db.transaction(async (tx) => {
            await Promise.all(transactions.map(async (transaction_data) => {
    ...
    async saveTransactionAndUpdateWallet(transaction_data) {
        await this.db.transaction(async (tx) => {
            const coin_id = await this.getCoinIdOfTransaction(transaction_data, tx);
            ...
            const [from_wallet] = await tx
                ...
                .from(schema.wallets)
                .where(and(eq(schema.wallets.address, transaction_data.from_address), eq(schema.wallets.blockchain, transaction_data.blockchain)));
            const [to_wallet] = await tx
                .select({ id: schema.wallets.id })
                .from(schema.wallets)
                .where(and(eq(schema.wallets.address, transaction_data.to_address), eq(schema.wallets.blockchain, transaction_data.blockchain)));
            ...
            if (from_wallet) {
                ...
                if (transaction_data.type === "native") {
                    ...
                    await tx
                        ...
                        .where(and(eq(schema.walletCoins.coin_id, coin_id), eq(schema.walletCoins.wallet_id, from_wallet.id)));
                }
            }
            ...
            if (to_wallet) {
                ...
                if (transaction_data.type === "native") {
                    ...
                    await tx
                        ...
                        .where(and(eq(schema.walletCoins.coin_id, coin_id), eq(schema.walletCoins.wallet_id, to_wallet.id)));
                }
            }
        });
    }
    async getTransactions(wallet_data, transactions_page) {
        const page_size = 20;
        const transactions = await this.db
            .select()
            .from(schema.transactions)
            .where(and(eq(schema.transactions.blockchain, wallet_data.blockchain), or(eq(schema.transactions.from_address, wallet_data.address), eq(schema.transactions.to_address, wallet_data.address))))
            .offset((transactions_page - 1) * page_size)
            .limit(page_size);
        return transactions;
    }
    async updateWalletBackfillStatus(wallet_data, status) {
        await this.db
            .update(schema.wallets)
            .set({ backfill_status: status })
            .where(and(eq(schema.wallets.address, wallet_data.address), eq(schema.wallets.blockchain, wallet_data.blockchain)));
    }
    // Helpers
    async getCoinIdOfTransaction(transaction_data, tx) {
        if (transaction_data.type === "native") {
            const [coin] = await tx
                .select({ id: schema.coins.id })
                .from(schema.coins)
                .where(and(eq(schema.coins.name, blockchains[transaction_data.blockchain].coin)))
                .limit(1);
            return coin.id;
        }
        else {
            const [coin] = await tx
                ...
                .where(and(eq(schema.contracts.blockchain, transaction_data.blockchain), eq(schema.contracts.contract_address, transaction_data.coin_address)))
                .limit(1);
            return coin.id;
        }
    }
