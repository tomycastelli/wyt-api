...
export class WalletsProviderAdapters {
    ethereumProvider;
    // private bitcoinProvider: BitcoinProvider;
    // private solanaProvider: SolanaProvider;
    constructor(moralis_api_key, blockchain_com_api_key, solana_rpc_endpoint) {
    ...
    async getRecentTransactions(wallet_data) {
        return this.ethereumProvider.getRecentTransactions(wallet_data);
    }
    async getTransactionHistory(wallet_data, loop_cursor) {
        return this.ethereumProvider.getTransactionHistory(wallet_data, loop_cursor);
    }
}
class EthereumProvider {
    ...
    async getWallet(address, blockchain) {
        ...
        let balances_data = await Moralis.EvmApi.wallets.getWalletTokenBalancesPrice({
        ...
        if (first_transaction.result[0]) {
            wallet_data.first_transfer_date = new Date(first_transaction.result[0].blockTimestamp);
        }
        return wallet_data;
    }
    async getRecentTransactions(wallet_data) {
        ...
        return this.transactionsFromWalletHistory(recent_transactions.result, wallet_data);
    }
    async getTransactionHistory(wallet_data, loop_cursor) {
        ...
        return {
            transactions: this.transactionsFromWalletHistory(transaction_history.result, wallet_data),
            cursor: transaction_history.pagination.cursor,
        };
    }
    transactionsFromWalletHistory(transaction_history_data, wallet_data) {
        const transactions_data = [];
        for (const tx of transaction_history_data.filter((rt) => !rt.possibleSpam)) {
            ...
            for (const erc20tx of tx.erc20Transfers) {
                transactions_data.push({
                    ...
                    fee: BigInt(Number(tx.transactionFee) *
